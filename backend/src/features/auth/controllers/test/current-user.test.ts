...
import { mockAuthRequest, mockAuthResponse, authUserPayload } from '@root/mocks/auth.mock';
...
jest.mock('@services/database/user.service');
...
describe('CurrentUser', () => {
  ...
  describe('token', () => {
    it('should set session token to null and send correct json response', async () => {
      const req: Request = mockAuthRequest({}, { username: USERNAME, password: PASSWORD }, authUserPayload) as unknown as Request;
    ...
    it('should set session token and send correct json response', async () => {
      const req: Request = mockAuthRequest({ jwt: '12djdj34' }, { username: USERNAME, password: PASSWORD }, authUserPayload) as unknown as Request;
      const res: Response = mockAuthResponse();
