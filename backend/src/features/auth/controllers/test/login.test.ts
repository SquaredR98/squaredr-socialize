...
describe('SignIn', () => {
  ...
  it('should throw an error if username is not available', () => {
    const req: Request = mockAuthRequest({}, { username: '', password: PASSWORD }) as unknown as Request;
  ...
  it('should throw an error if username length is less than minimum length', () => {
    const req: Request = mockAuthRequest({}, { username: WRONG_USERNAME, password: WRONG_PASSWORD }) as unknown as Request;
    const res: Response = mockAuthResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
  ...
  it('should throw an error if username length is greater than maximum length', () => {
    const req: Request = mockAuthRequest({}, { username: LONG_USERNAME, password: WRONG_PASSWORD }) as unknown as Request;
    const res: Response = mockAuthResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
      expect(error.serializeErrors().message).toEqual('Invalid username');
  ...
  it('should throw an error if password is not available', () => {
    const req: Request = mockAuthRequest({}, { username: USERNAME, password: '' }) as unknown as Request;
    const res: Response = mockAuthResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
  ...
  it('should throw an error if password length is less than minimum length', () => {
    const req: Request = mockAuthRequest({}, { username: USERNAME, password: WRONG_PASSWORD }) as unknown as Request;
    const res: Response = mockAuthResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
  ...
  it('should throw an error if password length is greater than maximum length', () => {
    const req: Request = mockAuthRequest({}, { username: USERNAME, password: LONG_PASSWORD }) as unknown as Request;
    const res: Response = mockAuthResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
      expect(error.serializeErrors().message).toEqual('Invalid password');
